...
class Wall {
  ...
  Wall(float x, float y, float sizeX, float sizeY) {
    ...
    //On détermine la taille car les walls au planchers ont une taille différente que ceux sur les côtés
    this.sizeX = sizeX;
    this.sizeY = sizeY;
  }
  ...
  void display(float scoreLow, float scoreMid, float scoreHi, float intensity, float scoreGlobal) {
    ...
    displayColor = color(scoreLow*0.5, scoreMid*0.5, scoreHi*0.5, scoreGlobal*0.25);
